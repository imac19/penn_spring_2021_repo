...
class slam_t:
    ...
    def stratified_resampling(p, w):
        ...
        for m in range(0, len(w)):
            ...
            while u > c:
                i+=1
                c+=w[i]
            new_location = [p[0][i], p[1][i], p[2][i]]
            new_particles[0][m] = new_location[0]
    ...
    def get_control(s, t):
        ...
        #### TODO: XXXXXXXXXXX
        state_current = s.lidar[t]['xyth']
        state_prev = s.lidar[t-1]['xyth']
        control = smart_minus_2d(state_current, state_prev)
    ...
    def dynamics_step(s, t):
        ...
        for i in range(0, s.n):
            p_update = [s.p[0][i], s.p[1][i], s.p[2][i]]
            p_update = smart_plus_2d(p_update, control)
            
            noise = np.random.multivariate_normal(np.zeros(len(s.Q)), s.Q)
            p_update = smart_plus_2d(p_update, noise)
    ...
    def observation_step(s, t):
        ...
        for i in range(0, s.n):
            ...
            points = s.rays2world(particle, distances, head_angle, neck_angle)
...
#                 if (x, y) in grid_cell_tuples:
#                     s.map.log_odds[x][y] += s.lidar_log_odds_occ
#                 else:
#                     s.map.log_odds[x][y] += s.lidar_log_odds_free
#                     
